                ...
                [
                    {
                        ...
                        "score": result.score,
                        "amScore": result.amScore,
                        "lmScore": result.lmScore,
                        "words": [
                            self.word_dict.get_entry(x) for x in result.words if x >= 0
                        ],
                        "wordNum": len([x for x in result.words if x >= 0]),
                        "frameNum": T,
                    }
...
class DropoutW2lKenLMDecoder(W2lKenLMDecoder):
    def __init__(self, args, tgt_dict):
        super().__init__(args, tgt_dict)
    ...
    def generate(self, models, sample, **unused):
        ...
        emissions_eval, emissions_1, emissions_2, emissions_3 = self.get_emissions(models, encoder_input)
        return (self.decode(emissions_eval), self.decode(emissions_1), self.decode(emissions_2), self.decode(emissions_3))

    def get_emissions(self, models, encoder_input):
        """Run encoder and normalize emissions"""
        model = models[0]
        model.eval()
        encoder_out_eval = model(**encoder_input)
        model.train()
        encoder_out_1 = model(**encoder_input)
        encoder_out_2 = model(**encoder_input)
        encoder_out_3 = model(**encoder_input)
        if self.criterion_type == CriterionType.CTC:
            emissions_eval = model.get_logits(encoder_out_eval) # no need to normalize emissions
            emissions_1 = model.get_logits(encoder_out_1)
            emissions_2 = model.get_logits(encoder_out_2)
            emissions_3 = model.get_logits(encoder_out_3)

        return emissions_eval.transpose(0, 1).float().cpu().contiguous(), emissions_1.transpose(0, 1).float().cpu().contiguous(), emissions_2.transpose(0, 1).float().cpu().contiguous(), emissions_3.transpose(0, 1).float().cpu().contiguous()
